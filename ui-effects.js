...
(function () {
  ...
  function renderEventsList(listEl, titleEl, events, category) {
    ...
    if (category) {
      ...
      if (filtered.length === 0 && events.length > 0) {
        // events already filtered by caller
        events.forEach(ev => {
      ...
      const cats = [...new Set(events.map(ev => ev.category))];
      cats.forEach(cat => {
        const heading = document.createElement("div");
        heading.className = "drawer-group-heading";
        heading.textContent = cat;
        listEl.appendChild(heading);
        events.filter(ev => ev.category === cat).forEach(ev => {
          listEl.appendChild(buildEventFlipCard(ev));
        });
      });
    }
  ...
  window.UIEffects = {
    playClickSound, playCyberBootSound, spawnSparkles, spawnElectricBurst,
    hideLoadingScreen, buildEventFlipCard, renderEventsList,
    initScrollReveal, initBackToTop, initScrollSpy
  };
